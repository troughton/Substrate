...
public final actor GPUResourceUploader {
    ...
    public static func generateMipmaps(for texture: Texture) async -> RenderGraphExecutionWaitToken {
        self.renderGraph.addPass(CallbackBlitRenderPass(name: "Generate Mipmaps for \(texture.label ?? "Texture(handle: \(texture.handle))")", resources: [.init(texture, [.blitSource, .blitDestination])]) { bce in
    ...
    public static func withUploadBuffer(length: Int, cacheMode: CPUCacheMode = .writeCombined, accessing resources: [ResourceUsage], fillBuffer:  @Sendable (UnsafeMutableRawBufferPointer, inout Range<Int>) throws -> Void, copyFromBuffer: @escaping @Sendable (_ buffer: Buffer, _ offset: Int, _ blitEncoder: BlitCommandEncoder) -> Void) async rethrows -> RenderGraphExecutionWaitToken {
        ...
        self.renderGraph.addBlitCallbackPass(name: "uploadBytes(length: \(length), cacheMode: \(cacheMode))", resources: resources + [
            .init(stagingBuffer, .blitSource, byteRange: stagingBufferOffset..<(stagingBufferOffset + length))
        ]) { bce in
    ...
    public static func uploadBytes(count: Int, to buffer: Buffer, offset: Int, _ bytes: (UnsafeMutableRawBufferPointer, inout Range<Int>) throws -> Void) async rethrows -> RenderGraphExecutionWaitToken {
        ...
        if buffer.storageMode == .shared || buffer.storageMode == .managed {
            ...
            self.renderGraph.addBlitCallbackPass(name: "uploadBytes(count: \(count), to: \(buffer), offset: \(offset))", resources: [
                .init(stagingBuffer, .blitSource, byteRange: stagingBufferOffset..<(stagingBufferOffset + count)),
                .init(buffer, .blitDestination, byteRange: offset..<(offset + count))
            ]) { bce in
    ...
    public static func replaceTextureRegion(_ region: Region, mipmapLevel: Int, slice: Int, in texture: Texture, withBytes bytes: UnsafeRawPointer, bytesPerRow: Int, bytesPerImage: Int) async -> RenderGraphExecutionWaitToken {
        ...
        if texture.storageMode == .shared || texture.storageMode == .managed {
            ...
            resources: [
                .init(stagingBuffer, .blitSource, byteRange: stagingBufferOffset..<(stagingBufferOffset + bytesPerImage)),
