...
public func NSStringFromClass(_ aClass: AnyClass) -> String {
    let aClassName = String(reflecting: aClass)
...
public func NSClassFromString(_ aClassName: String) -> AnyClass? {
    ...
    case 1:
        guard !aClassName.hasPrefix("_Tt") else {
            print("*** NSClassFromString(\(aClassName)): cannot yet decode mangled class names")
            return nil
        }
    ...
    default:
        print("*** NSClassFromString(\(aClassName)): nested class names not yet supported")
        return nil
    }
