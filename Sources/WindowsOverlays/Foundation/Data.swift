...
#if os(Windows)
    fatalError("Mapped memory is not implemented on Windows.")
#else
    munmap(mem, length)
#endif
}
...
public final class _DataStorage {
    ...
    public init(bytes: UnsafeMutableRawPointer?, length: Int, copy: Bool, deallocator: ((UnsafeMutableRawPointer, Int) -> Void)? = nil, offset: Int = 0) {
        precondition(length < _DataStorage.maxSize)
        _offset = offset
        if length == 0 {
            _capacity = 0
    ...
    public func withInteriorPointerReference<T>(_ range: Range<Int>, _ work: (Data) throws -> T) rethrows -> T {
        if range.isEmpty {
            return try work(Data()) // zero length data can be optimized as a singleton
        }
        
        return try work(Data(bytesNoCopy: _bytes!.advanced(by: range.lowerBound - _offset), count: range.count, deallocator: .none))
    }
...
public struct Data : Equatable, Hashable, RandomAccessCollection, MutableCollection, RangeReplaceableCollection {
    ...
    public enum Deallocator {
        ...
        fileprivate var _deallocator : ((UnsafeMutableRawPointer, Int) -> Void) {
            ...
            switch self {
            case .virtualMemory:
                return { DataDeallocatorVM($0, $1) }
            case .unmap:
                return { DataDeallocatorUnmap($0, $1) }
            case .free:
                return { DataDeallocatorFree($0, $1) }
            case .none:
                return { _, _ in }
            case .custom(let b):
                return { (ptr, len) in
    ...
    ///
    /// - parameter bytes: An array of bytes to copy.
    public init(bytes: ContiguousArray<CChar>) {
        let count = bytes.count
        _backing = bytes.withUnsafeBufferPointer {
            return _DataStorage(bytes: $0.baseAddress, length: count)
        }
    ...
    public init(bytes: UnsafeMutableRawPointer, length: Int, copy: Bool, deallocator: ((UnsafeMutableRawPointer, Int) -> Void)? = nil, offset: Int = 0) {
        _backing = _DataStorage(bytes: bytes, length: length, copy: copy, deallocator: deallocator, offset: offset)
        _sliceRange = 0..<length
    }
    ...
    public init<S: Sequence>(_ elements: S) where S.Iterator.Element == UInt8 {
        if elements is Array<UInt8> {
            ...
            let (endIterator, _) = UnsafeMutableBufferPointer(start: _backing.mutableBytes?.assumingMemoryBound(to: UInt8.self), count: underestimatedCount).initialize(from: elements)
    ...
    public var hashValue: Int {
        ...
        _withStackOrHeapBuffer(hashRange.count + 1) { buffer in
            ...
            hashValue = Int(CFHashBytes(buffer.pointee.memory.assumingMemoryBound(to: UInt8.self), hashRange.count))
        }
        return hashValue
    }
    ...
    public struct Iterator : IteratorProtocol {
        ...
        public mutating func next() -> UInt8? {
            ...
            var buffer = _buffer
            return withUnsafeMutablePointer(to: &buffer) { ptr_ in
