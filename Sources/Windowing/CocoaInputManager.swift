...
public final class CocoaInputManager : InputManagerInternal {
    ...
    public func update(windows: [Window]) {
        ...
        case .resizeNESW: // FIXME: we should remove this private methods before release.
        ...
        if self.inputState[.keyboardScanCode][.command].isActive(frame: self.frame), self.inputState[.keyboardScanCode][.keyQ].isActive(frame: self.frame) {
            NSApp.perform(#selector(NSApp.terminate), with: nil, afterDelay: 0.05)
        }
    }
    ...
    private func handleEvents(windows: [Window]) {
        ...
        for event in self.eventQueue {
            ...
            case .mouseMoved:
                ...
                inputState[.mouse][.mouseXInWindow] = RawInputState(value: Float(locationInWindow.x), frame: self.frame)
                inputState[.mouse][.mouseYInWindow] = RawInputState(value: Float(locationInWindow.y), frame: self.frame)
        ...
        inputState[.mouse][.mouseXRelative] = RawInputState(value: Float(mouseRelativeX), frame: self.frame)
        inputState[.mouse][.mouseYRelative] = RawInputState(value: Float(mouseRelativeY), frame: self.frame)
