...
public final class SDLInputManager : InputManager {
    public internal(set) var inputState = InputState<RawInputState>()
    public private(set) var frame : UInt32 = 1
    ...
    public func setupImGui() {
        ...
        #else
        let keyCount = Int(ImGuiKey_COUNT)
        #endif
    ...
    private func handleEvents(windows: [Window]) {
        let activeState = RawInputState(active: true, frame: self.frame)
        ...
        while SDL_PollEvent(&event) != 0 {
            ...
            if event.type == SDL_WINDOWEVENT.rawValue {
                ...
                case SDL_MOUSEMOTION:
                    ...
                    inputState[.mouseX] = RawInputState(value: Float(motion.x), frame: self.frame)
                    inputState[.mouseY] = RawInputState(value: windowHeight - Float(motion.y), frame: self.frame)
                    inputState[.mouseXInWindow] = RawInputState(value: Float(motion.x), frame: self.frame)
                    inputState[.mouseYInWindow] = RawInputState(value: windowHeight - Float(motion.y), frame: self.frame)
                    inputState[.mouseXRelative] = RawInputState(value: Float(motion.xrel), frame: self.frame)
                    inputState[.mouseYRelative] = RawInputState(value: Float(motion.yrel), frame: self.frame)
                    
                case SDL_MOUSEWHEEL:
                    let scroll = event.wheel
                    inputState[.mouseScrollX] = RawInputState(value: Float(scroll.x), frame: self.frame)
                    inputState[.mouseScrollY] = RawInputState(value: Float(scroll.y), frame: self.frame)
                ...
                case SDL_CONTROLLERAXISMOTION:
                    ...
                    if let inputSource = InputSource(fromGameControllerAxis: axisId) {
                        ...
                        if (value > JoyStickDeadZone || value < -JoyStickDeadZone) {
                            ...
                            inputState[.gamepad(slot: gamepadSlot)][inputSource] = RawInputState(value: normalizedValue, frame: self.frame)
                        } else {
                            inputState[.gamepad(slot: gamepadSlot)][inputSource].markInactive()
                        }
                    }
