...
public protocol InputLayer {
    func processInput(rawInput: inout InputState<RawInputState>, frame: UInt32)
}
...
public final class ConfigurableInputLayer<ActionType : InputActionType> : InputLayer {
    let modifiers : [InputModifier]
    let mappings : [InputLayerMapping<ActionType>]
    
    var transitionState = InputState<InputSourceTransitionState>()
    private var actionState = [ActionType : InputActionState]()
    
    init(modifiers: [InputModifier], mappings: [InputLayerMapping<ActionType>]) {
    ...
    public func processInput(rawInput: inout InputState<RawInputState>, frame: UInt32) {
...
extension ConfigurableInputLayer : Codable {
    ...
    public static func load(fileAt path: String) throws -> ConfigurableInputLayer<ActionType> {
        let fileContents = try Data(contentsOf: URL(fileURLWithPath: path))
        
        let decoder = JSONDecoder()
        return try decoder.decode(ConfigurableInputLayer<ActionType>.self, from: fileContents)
    }
}
