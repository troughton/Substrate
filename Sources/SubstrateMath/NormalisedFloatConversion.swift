...
@_specialize(kind: full, where I == Int16)
@inlinable
public func floatToSnorm<I: BinaryInteger & FixedWidthInteger & SignedInteger>(_ c: SIMD2<Float>, type: I.Type = I.self) -> SIMD2<I> {
...
@_specialize(kind: full, where I == Int16)
@inlinable
public func floatToSnorm<I: BinaryInteger & FixedWidthInteger & SignedInteger>(_ c: SIMD3<Float>, type: I.Type = I.self) -> SIMD3<I> {
    if any(c .!= c) { // Check for NaN – this check is faster than c.isNaN
        return .zero
    }
...
@_specialize(kind: full, where I == Int16)
@inlinable
public func floatToSnorm<I: BinaryInteger & FixedWidthInteger & SignedInteger>(_ c: SIMD4<Float>, type: I.Type = I.self) -> SIMD4<I> {
    if any(c .!= c) { // Check for NaN – this check is faster than c.isNaN
        return .zero
    }
...
@_specialize(kind: full, where I == UInt16)
@inlinable
public func floatToUnorm<I: BinaryInteger & FixedWidthInteger & UnsignedInteger>(_ c: SIMD2<Float>, type: I.Type = I.self) -> SIMD2<I> {
    if any(c .!= c) { // Check for NaN – this check is faster than c.isNaN
        return .zero
    }
...
@_specialize(kind: full, where I == UInt16)
@inlinable
public func floatToUnorm<I: BinaryInteger & FixedWidthInteger & UnsignedInteger>(_ c: SIMD3<Float>, type: I.Type = I.self) -> SIMD3<I> {
    if any(c .!= c) { // Check for NaN – this check is faster than c.isNaN
        return .zero
    }
    let c = c.clamped(lowerBound: .zero, upperBound: .one)
...
@_specialize(kind: full, where I == UInt16)
@inlinable
public func floatToUnorm<I: BinaryInteger & FixedWidthInteger & UnsignedInteger>(_ c: SIMD4<Float>, type: I.Type = I.self) -> SIMD4<I> {
    if any(c .!= c) { // Check for NaN – this check is faster than c.isNaN
        return .zero
    }
    let c = c.clamped(lowerBound: .zero, upperBound: .one)
